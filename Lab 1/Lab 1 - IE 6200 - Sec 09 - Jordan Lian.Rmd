...
## Task 1: Use the getwd() function in R to show your current working directory.
```{r Task 1}
getwd()
```
## Task 2: Create 2 variables given below and find the class of these 2 variables
### a.	x <- 1: numeric class
```{r, x}
x <- 1
class(x)
...
## Task 3: Create a numerical vector "vect" with elements {9, 8, 7, 6, 4} of length 5. Using vector indexing show how you can display the third element in this vector in two different ways.
```{r Task 3, vect}
vect <- c(9, 8, 7, 6, 4)
...
## Task 5: Use the iris data frame that is available in R and display the first and last 5 rows of the data frame.
```{r Task 5}
head(iris, 5)
tail(iris, 5)
```
